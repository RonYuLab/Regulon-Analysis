...
def correlate_regulons(wt_auc,mut_auc,adata:list=None,plot:bool=False,subtract:bool=False,sort_by:str=None):
    '''
    From input of two AUCell matrices. (WT,Mutant) Function will correlate regulons together, then compare to correlation pattern in mutant genotype.
    Lastly, a condensed matrix diagonally split will visualize subtracted pearson between conditions (Wt - Mutant & Mutant - Wt) Output stores the matrices in list.
    [0] = Wt
    [1] = Mut
    [2] = Subtracted (WT-Mut)
    [3] = Inverse Subtracted (Mut-WT)
    [4] = Split Subtracted ([2]/[3])
    '''
    auc_list = [wt_auc,mut_auc] 
    # If you require sorting. Supply an adata reference. 
    if sort_by != None: 
        if adata == None:
            print('Adata objects are required to sort cells in AUC matrices.')
        if sort_by == None:
            print('Supply an obs variable to sort cells by.') 
            raise
        
        print('Sorting AUCell Matrices Prior to Correlation')
    else:
        cormats = []
        # Convert to correlation matrix. Values are pearson coefficient. 
        for mtx in auc_list: 
            cor = mtx.corr() 
            cormats.append(cor) 
        # Cluster the correlations and store the order embeddings to apply to Mut. mtx.
        g = sns.clustermap(cormats[0],xticklabels = True, yticklabels = True)
        plt.close(g.fig)
        orderings = [g.dendrogram_row.reordered_ind, g.dendrogram_col.reordered_ind] # Dendrogram embeddings accessible from graph object.
        ordered_wt_mtx = cormats[0].iloc[orderings[0],orderings[1]]
        ordered_mut_mtx = cormats[1].iloc[orderings[0],orderings[1]]
        cormats[0] = ordered_wt_mtx
        cormats[1] = ordered_mut_mtx
    if subtract:
        # Create base subtracted matrices
        subtracted_mtx = ordered_wt_mtx - ordered_mut_mtx 
        cormats.append(subtracted_mtx)
        inverse_subtracted_mtx = ordered_mut_mtx - ordered_wt_mtx
        cormats.append(inverse_subtracted_mtx)

        # Create inverse-split subtracted matrix
        assert cormats[2].shape == cormats[1].shape, "DataFrames must be the same shape"
        m1 = cormats[2].to_numpy()
        m2 = cormats[3].to_numpy()
        n = m1.shape[0]
        upper_mask = np.triu_indices(n,1)
        m1[upper_mask] = m2[upper_mask]
        cormats.append(m1)

    if plot:
        # WT
        plt.figure(figsize = (10,10)) 
        sns.heatmap(cormats[0], xticklabels=True, yticklabels=True,vmin=-1,vmax=1)
        plt.title("WT Regulon Correlations") 
        plt.show() 
    
        # KO
        plt.figure(figsize = (10,10)) 
        sns.heatmap(cormats[1], xticklabels=True, yticklabels=True,vmin=-1,vmax=1)
        plt.title("Fzd1cKO Regulon Correlations") # Optional title
        plt.show()  
    
        # Split Subtracted (WT - Fzd1cKO = Bottom | Fzd1cKO - WT = TOP) 
        plt.figure(figsize = (10,10)) 
        g = sns.heatmap(cormats[4], xticklabels=True, yticklabels=True,vmin=-0.5,vmax=0.5,cmap='icefire')
        
        # Adjust matrix to have regulon labels (Using first matrix labels as a reference)
        labels = [item.get_text() for item in g.get_xticklabels()]
        labels = list(cormats[0].index)
        g.set_xticklabels(labels) 
        labels = list(cormats[0].columns)
        g.set_yticklabels(labels)
        plt.title("WT-KO/KO-WT Regulon Correlations") # Optional title
        plt.show()
